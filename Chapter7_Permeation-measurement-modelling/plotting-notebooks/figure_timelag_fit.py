...
def plot_timelag_flux_fit_hdpe(
    data_folder_path,
    width=6., height=4.0,
    x_lo=0., y_lo=0.,
    model_colour='C2',
    ...
    for file in ordered_hdpe_files:
        ...
        # Load fit data
        fit_files = [f for f in os.listdir(data_dir) if f.startswith('flux_data') and f.endswith(('.csv'))]
    ...
    for i, exp_name in enumerate(ordered_hdpe_files):
        ...
        # Plot exp data
        ax.plot(exp_data['time'], exp_data['flux'], color=exp_colour, marker=symbol, markersize=markersize, linestyle=linestyle, linewidth=linewidth, label='Exp.')
        
        # Plot fit data
        ax.plot(fit_data['time'], fit_data['flux'], color=model_colour, marker=symbol, markersize=markersize, linestyle=linestyle, linewidth=linewidth, label='Model')
    ...
    if save_fig:
        ...
        filename = f"fig_timelag_flux_fit_hdpe.{save_format}"
...
def plot_timelag_flux_fit_peeka(
    data_folder_path,
    width=6., height=4.0,
    x_lo=0., y_lo=0.,
    model_colour='C2',
    ...
    if save_fig:
        os.makedirs(folder_to_save, exist_ok=True)
        filename = f"fig_timelag_flux_fit_peeka.{save_format}"
        filepath = os.path.join(folder_to_save, filename)
        fig.savefig(filepath, format=save_format, dpi=dpi, bbox_inches='tight')
        print(f"Figure saved to {filepath}")
