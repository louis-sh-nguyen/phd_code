...
def plot_timelag_diffusivity(
    ...
    for pressure in unique_pressures_hdpe:
        ...
        diffusivity_values = group['diffusivity / cm^2 s^-1']

        # Dictionary to track occurrences of each (temp, pressure) combination
        temp_pressure_count = {}
        
        for temp, diff in zip(group['temperature / C'], diffusivity_values):
            ...
            ax1.scatter(temp, diff,
                       marker=pressure_marker_map[pressure],
                       color=color,
                       facecolor=markerfacecolor)
        
    ax1.set_ylim(bottom=0., top=1.5e-6)
    ax1.set_xlabel('Temperature / °C')
    ax1.set_ylabel(r'$D_\mathrm{TL}$ / $\mathrm{cm^{2} \, s^{-1}}$')
    ...
    if save_fig:
        ...
        filename = f"fig_timelag_diffusivity.{save_format}"
