...
def plot_total_density_SW_vertical(
        data_path,
        width=5., height=3.5,
    ...
    for i, (T, ax, title) in enumerate(zip(T_celsius_values, axs, titles)):
        ax.plot(df[T]['p / bar']*barToMPa, df[T]['rho_t(TPS) / g/m3'] / m3ToCm3, 
    ...
    #-------------------------------
    # Labels
    ax1.set_ylabel(r'$\rho_{\mathrm{t}}$ / $\mathrm{g \, cm^{-3}}$')
    ax2.set_ylabel(r'$\rho_{\mathrm{t}}$ / $\mathrm{g \, cm^{-3}}$')
    ax3.set_ylabel(r'$\rho_{\mathrm{t}}$ / $\mathrm{g \, cm^{-3}}$')
    ax3.set_xlabel(r'$p$ / MPa')
    
    # Save figure
    if save_fig == True:
        save_fig_path = folder_to_save + f"/fig_total_density_SW.{save_format}"
