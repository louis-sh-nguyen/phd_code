...
def plot_solvent_volume_SW_vertical(
        data_path,
        width=5., height=3.5,
    ...
    for i, (T, ax, title) in enumerate(zip(T_celsius_values, axs, titles)):
        ax.plot(df[T]['p / bar']*barToMPa, df[T]['Vs / m3/g'] * m3ToCm3, 
    ...
    #-------------------------------
    # Labels
    ax1.set_ylabel(r'$\hat{V}_{\mathrm{s}}$ / $\mathrm{cm^3 \, g^{-1}}$')
    ax2.set_ylabel(r'$\hat{V}_{\mathrm{s}}$ / $\mathrm{cm^3 \, g^{-1}}$')
    ax3.set_ylabel(r'$\hat{V}_{\mathrm{s}}$ / $\mathrm{cm^3 \, g^{-1}}$')
    ax3.set_xlabel(r'$p$ / MPa')
    
    # Save figure
    if save_fig == True:
        save_fig_path = folder_to_save + f"/fig_solvent_volume_SW.{save_format}"
